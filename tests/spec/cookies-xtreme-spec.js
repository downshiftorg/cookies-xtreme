...
describe('UNIT TESTS', function () {
    ...
    describe('Events', function () {
      ...
      describe('addListener(key, listener)', function () {
        it('should add a listener with the given key', function () {
          Cookies.addListener('something', listener);

          expect(Cookies.listeners['something'][0]).toBe(listener);
        });
      });

      describe('removeListener(key, listener)', function () {
        beforeEach(function () {
          Cookies.addListener('something', listener);
        });

        it('should remove the given listener', function () {
          Cookies.removeListener('something', listener);
          expect(Cookies.listeners['something'].length).toBe(0);
        });
      });

      describe('removeAllListeners(key)', function () {
        it('should remove all listeners of the given key', function () {
          Cookies.addListener('something', listener);
          Cookies.removeAllListeners('something');
          expect(Cookies.listeners.length).toBe(0);
        });
      });
    });
