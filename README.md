...
Cookies.js (XTREME VERSION!!!) is a small client-side javascript library that makes managing cookies easy. It is a fork of the super rad library [Cookies.js](https://github.com/ScottHamper/Cookies). The purpose of this library is to make Cookies.js event driven so you can monitor changes to cookies.
...
## Getting the Library
#### Direct downloads
- [v1.1.0 Minified](https://raw.github.com/downshiftorg/cookies-xtreme/1.1.0/dist/cookies-xtreme.min.js) (~1 KB gzipped)
- [v1.1.0 Unminified](https://raw.github.com/downshiftorg/cookies-xtreme/1.1.0/dist/cookies-xtreme.js) (~1.7 KB gzipped)
...
[Cookies.addListener(key, listener)](#cookiesaddlistenerkey-listener)  
[Cookies.removeListener(key, listener)](#cookiesremovelistenerkey-listener)  
[Cookies.removeAllListeners(key)](#cookiesremovealllistenerskey)  
[Cookies.emit(key, data)](#cookiesemitkey-data)  
...
Adds a listener for the given key. When a cookie value is set, an event of the form
`set.key` - where `key` is the cookie key - will be emitted.
...
Trigger an event for the given key. This is best reserved for internal use. All calls
to `Cookies.set` or it's alias will result in an event of `set.key` - where key is the cookie
key.
